...
@Composable
fun OneSpaceCaptureCard(navController: NavHostController, spaceViewModel: SpaceViewModel) {
    var isScannedQRCode by remember { mutableStateOf(false) }

    val spaceList = spaceViewModel.spaceState.spaces.map { it.spaceID }
            ...
            if (!isScannedQRCode) {
                ...
                qrValue?.let {
                    ...
                    if (isNumber(idStr)) {
                        ...
                        if (it.startsWith("1") && spaceList.contains(id)) {
                            isScannedQRCode = true
                            navController.navigate(InventoryManagerScreen.ViewSpaceInfo.name)

                            spaceViewModel.recordSingleCapture(id.toString())
                        }
                    }
                }
            }
...
@Composable
fun PreviewSpace(onQRScanned: (Barcode?) -> Unit) {
